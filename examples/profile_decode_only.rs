...
fn main() {
    ...
    let webp_path = std::env::args().nth(1).unwrap_or("/tmp/profile_test.webp".to_string());
    
    let webp_data = if std::path::Path::new(&webp_path).exists() {
        fs::read(&webp_path).expect("Failed to read webp")
    } else {
        // Encode from PNG
        let png_path = std::env::args().nth(2).unwrap_or("/tmp/clic2025/11f2b039b293758398b1a7a8afa64bb2.png".to_string());
