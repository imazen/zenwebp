fn main() {
    ...
    let webp = zenwebp::EncodeRequest::new(&config, &rgba, zenwebp::PixelLayout::Rgba8, info.width, info.height)
        .encode().unwrap();
    // Save for C decoder
    std::fs::write("/tmp/codec_wiki_lossless.webp", &webp).unwrap();
    eprintln!("encoded {}x{} lossless: {} bytes", info.width, info.height, webp.len());
    // Decode 5 times
    for _ in 0..5 {
