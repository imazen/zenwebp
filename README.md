...
Benchmarks compare against libwebp (C reference) with runtime SIMD dispatch (no
`-C target-cpu=native` — that is what ships to users). Full methodology, harnesses,
competitor pinning, and result files are under
[`benchmarks/`](https://github.com/imazen/zenwebp/tree/main/benchmarks) and
[`docs/PERFORMANCE.md`](https://github.com/imazen/zenwebp/blob/main/docs/PERFORMANCE.md).

### Lossy decoder

**Bit-exact with libwebp** — 0 pixel diffs on 12,825 scraped WebP files and 218
conformance files. The v2 streaming decoder (`src/decoder/vp8v2/`) is the default
path, byte-identical to v1 on all conformance + roundtrip tests.

Tested across 14 images (CLIC2025 photos, screenshots, CID22):

| Content | zenwebp vs libwebp (C) |
|---------|------------------------|
...
| Method | Speed vs libwebp | Size vs libwebp |
|--------|------------------|-----------------|
...
24/24 pixel-exact lossless roundtrips verified. At the same quality setting,
zenwebp produces files within 1–5% of libwebp's size with comparable visual quality.
<!-- crates.io:skip-end -->
