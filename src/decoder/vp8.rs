...
pub struct Vp8Decoder<'a> {
    ...
    threading: super::api::DecoderThreading,
}
...
impl<'a> Vp8Decoder<'a> {
    ...
    pub(crate) fn new(data: &'a [u8]) -> Self {
        ...
        Self {
            ...
            threading: super::api::DecoderThreading::Auto,
        }
    }
    ...
    /// Precompute filter parameters for all macroblocks in a row.
    /// Returns per-MB (filter_level, interior_limit, hev_threshold, do_subblock_filtering).
    #[cfg(feature = "std")]
    fn precompute_filter_params(&self, mby: usize) -> RowFilterParams {
        let mbwidth = self.mbwidth as usize;
        let mb_row_start = mby * mbwidth;
        let mb_params: Vec<MbFilterParams> = (0..mbwidth)
            .map(|mbx| {
        ...
        RowFilterParams {
            mbwidth,
            mby,
            extra_y_rows: self.extra_y_rows,
            cache_y_stride: self.cache_y_stride,
            cache_uv_stride: self.cache_uv_stride,
            filter_type: self.frame.filter_type,
            mb_params,
        }
    }

    /// Precompute per-MB dither amplitudes for a row.
    #[cfg(feature = "std")]
    fn precompute_dither_amps(&self, mby: usize) -> Vec<i32> {
        let mbwidth = self.mbwidth as usize;
        let mb_row_start = mby * mbwidth;
        (0..mbwidth)
            .map(|mbx| {
    ...
    /// Output a row from external cache buffers to frame.
    /// Used by the threaded path where the worker returns filtered cache buffers.
    #[cfg(feature = "std")]
    fn output_row_from_external_cache(
        &mut self,
        mby: usize,
        ext_cache_y: &[u8],
        ext_cache_u: &[u8],
        ...
        for y in 0..num_y_rows {
            ...
            let dst_start = dst_row * luma_w;
            self.frame.ybuf[dst_start..dst_start + luma_w]
                .copy_from_slice(&ext_cache_y[src_start..src_start + luma_w]);
        }
        ...
        for y in 0..num_uv_rows {
            ...
            let dst_start = dst_row * chroma_w;
            self.frame.ubuf[dst_start..dst_start + chroma_w]
                .copy_from_slice(&ext_cache_u[src_start..src_start + chroma_w]);
            self.frame.vbuf[dst_start..dst_start + chroma_w]
                .copy_from_slice(&ext_cache_v[src_start..src_start + chroma_w]);
        }
    }
    ...
    fn decode_frame_(mut self) -> Result<Frame, DecodeError> {
        ...
        #[cfg(feature = "std")]
        let use_threading = match self.threading {
            super::api::DecoderThreading::Auto => self.frame.width >= 512 && self.mbheight > 1,
            super::api::DecoderThreading::SingleThreaded => false,
        };
        #[cfg(not(feature = "std"))]
        ...
        #[cfg(feature = "std")]
        if use_threading {
            return self.decode_frame_threaded();
        }
        // Suppress unused variable warning in no_std builds
        let _ = use_threading;
    ...
    /// 2-thread pipelined decode: main thread reconstructs row N+1 while a worker
    /// thread filters+dithers row N. The worker returns its cache buffers so the
    /// main thread can output to frame and reuse them for the next iteration.
    ///
    /// Critical ordering: the extra rows (filter border context) must come from the
    /// FILTERED previous row, not the unfiltered one. The filter modifies pixels near
    /// MB boundaries, and the next row's filter reads those modified pixels.
    ///
    /// Pipeline per row:
    /// 1. Reconstruct row N into self.cache (extra area irrelevant for prediction)
    /// 2. Join worker for row N-1, get filtered cache back
    /// 3. Output row N-1 from filtered cache to frame
    /// 4. Copy FILTERED bottom border from worker's cache into self.cache's extra area
    /// 5. Give self.cache (now with correct extra area) to a new worker for filtering
    /// 6. Swap: self.cache gets the recycled worker cache (ready for next reconstruction)
    #[cfg(feature = "std")]
    fn decode_frame_threaded(mut self) -> Result<Frame, DecodeError> {
        // Summon SIMD token once for the entire decode
        let simd_token: SimdTokenType = {
            #[cfg(all(feature = "simd", any(target_arch = "x86_64", target_arch = "x86")))]
            {
        ...
        // Allocate second cache set for double-buffering
        let cache_y_size = self.cache_y.len();
        let cache_uv_size = self.cache_u.len();
        let mut alt_cache_y = vec![128u8; cache_y_size];
        let mut alt_cache_u = vec![128u8; cache_uv_size];
        let mut alt_cache_v = vec![128u8; cache_uv_size];

        let dither_enabled = self.dither_enabled;
        let mbheight = self.mbheight as usize;

        std::thread::scope(|scope| -> Result<(), DecodeError> {
            type WorkerResult = (Vec<u8>, Vec<u8>, Vec<u8>, super::dither::VP8Random, usize);
            let mut worker_handle: Option<std::thread::ScopedJoinHandle<'_, WorkerResult>> = None;

            for mby in 0..mbheight {
                let p = mby % self.num_partitions as usize;
                self.left = PreviousMacroBlock::default();

                // Phase 1: Reconstruct row mby into self.cache
                // The extra area of self.cache is either:
                //   - 128-fill (first row, same as single-threaded)
                //   - Filtered border from previous row (set in phase 4 of previous iteration)
                for mbx in 0..self.mbwidth as usize {
                    let mut mb = self.read_macroblock_header(mbx)?;
                ...
                // Phase 2+3: Join previous worker, output its results, copy filtered border
                if let Some(handle) = worker_handle.take() {
                    let (ret_y, ret_u, ret_v, ret_rg, ret_mby) = handle.join().unwrap();
                    // Output the filtered row to frame
                    self.output_row_from_external_cache(ret_mby, &ret_y, &ret_u, &ret_v);

                    // Phase 4: Copy FILTERED bottom border into self.cache's extra area.
                    // The filter modifies pixels near MB boundaries, so we must use
                    // the post-filter version for the next row's filter context.
                    rotate_extra_rows_to(
                        &ret_y,
                        &mut self.cache_y,
                        self.extra_y_rows,
                        self.cache_y_stride,
                    ...
                    // Recycle returned caches
                    alt_cache_y = ret_y;
                    alt_cache_u = ret_u;
                    alt_cache_v = ret_v;
                    self.dither_rg = ret_rg;
                }

                // Phase 5: self.cache now has correct extra area + reconstructed data.
                // Move it to the worker for filtering.
                // Swap with alt: self.cache goes to alt (for worker), alt goes to self (for next row).
                core::mem::swap(&mut self.cache_y, &mut alt_cache_y);
                core::mem::swap(&mut self.cache_u, &mut alt_cache_u);
                core::mem::swap(&mut self.cache_v, &mut alt_cache_v);

                // Precompute filter params and dither amps for this row
                let filter_params = self.precompute_filter_params(mby);
                let dither_amps = if dither_enabled {
                    self.precompute_dither_amps(mby)
                } else {
                    Vec::new()
                };

                // Take ownership of the cache for the worker (alt now holds reconstructed data)
                let mut work_y = core::mem::take(&mut alt_cache_y);
                let mut work_u = core::mem::take(&mut alt_cache_u);
                let mut work_v = core::mem::take(&mut alt_cache_v);
                let mut work_rg =
                    core::mem::replace(&mut self.dither_rg, super::dither::VP8Random::new());

                let work_mby = mby;
                let work_dither_enabled = dither_enabled;
                let work_cache_uv_stride = self.cache_uv_stride;
                let work_extra_uv_rows = self.extra_y_rows / 2;

                // Phase 6: Spawn worker to filter+dither the current row
                worker_handle = Some(scope.spawn(move || {
                    filter_row_free(
                        &mut work_y,
                        &mut work_u,
                        &mut work_v,
                    ...
                    if work_dither_enabled {
                        super::dither::dither_row(
                            &mut work_rg,
                            super::dither::DitherRowParams {
                                cache_u: &mut work_u,
                                cache_v: &mut work_v,
                                cache_uv_stride: work_cache_uv_stride,
                                extra_uv_rows: work_extra_uv_rows,
                                mb_dither_amps: &dither_amps,
                            },
                        );
                    }

                    (work_y, work_u, work_v, work_rg, work_mby)
                }));
...
/// Per-macroblock filter parameters, precomputed for the worker thread.
#[cfg(feature = "std")]
struct MbFilterParams {
    filter_level: u8,
    interior_limit: u8,
    hev_threshold: u8,
    do_subblock_filtering: bool,
}

/// Precomputed filter parameters for an entire macroblock row.
#[cfg(feature = "std")]
struct RowFilterParams {
    mbwidth: usize,
    mby: usize,
    extra_y_rows: usize,
    ...
    filter_type: bool,
    mb_params: Vec<MbFilterParams>,
}

/// Free function version of `filter_row_in_cache` that operates on external cache buffers
/// with precomputed filter parameters. Used by the threaded decode path.
#[cfg(feature = "std")]
fn filter_row_free(
    cache_y: &mut [u8],
    cache_u: &mut [u8],
    ...
    let mby = params.mby;
    let extra_y_rows = params.extra_y_rows;
    let extra_uv_rows = extra_y_rows / 2;
    let cache_y_stride = params.cache_y_stride;
    let cache_uv_stride = params.cache_uv_stride;

    for mbx in 0..params.mbwidth {
        let mbp = &params.mb_params[mbx];
        let filter_level = mbp.filter_level;

        if filter_level == 0 {
            continue;
        }

        let interior_limit = mbp.interior_limit;
        let hev_threshold = mbp.hev_threshold;
        let mbedge_limit = (filter_level + 2) * 2 + interior_limit;
        let sub_bedge_limit = (filter_level * 2) + interior_limit;
        let do_subblock_filtering = mbp.do_subblock_filtering;
...
/// Copy bottom border rows from source cache to the extra area (top) of destination cache.
/// Used for cross-cache rotation in the threaded decode path.
///
/// Cache layout: `[extra_rows][pixel_rows]` where pixel_rows = 16 (luma) or 8 (chroma).
/// Copies the bottom `extra_rows` of the pixel area to `dst` rows `0..extra_rows`.
#[cfg(feature = "std")]
fn rotate_extra_rows_to(src: &[u8], dst: &mut [u8], extra_rows: usize, stride: usize) {
    if extra_rows == 0 {
        return;
    }
    // Derive pixel_rows from the buffer size: total_rows = (len - FILTER_PADDING) / stride
    let total_rows = (src.len().saturating_sub(FILTER_PADDING)) / stride.max(1);
    let pixel_rows = total_rows - extra_rows;
    let src_start = pixel_rows * stride;
    let copy_size = extra_rows * stride;
    dst[..copy_size].copy_from_slice(&src[src_start..src_start + copy_size]);
}
