...
pub struct Vp8lConfig {
    ...
    /// Color cache bits (0 = disabled, 1-11 = enabled).
    pub cache_bits: u8,
...
impl Default for Vp8lConfig {
    fn default() -> Self {
        Self {
            ...
            cache_bits: 0, // Disabled by default, auto-detect later
            use_predictor: true,
            use_cross_color: false, // Start without cross-color
            use_subtract_green: true,
            use_palette: true,
            use_meta_huffman: false, // Start without meta-huffman
            predictor_bits: 9, // Use large blocks (512x512) for simpler encoding
            cross_color_bits: 4,
        }
    }
}
