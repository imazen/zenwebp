...
fn reference_test_with_options(
    ...
    if !decoder.is_lossy() {
        ...
        // Lossy path: the yuv crate's bilinear chroma upsampling differs from
        // libwebp's "fancy" upsample ((3*near + far + 2) >> 2). The reference
        // images were generated by dwebp, so per-channel differences can reach
        // ~200 due to different chroma interpolation + coefficient precision.
        // We verify the mean absolute error is small instead.
