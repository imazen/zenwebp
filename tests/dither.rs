...
//! then decodes with dithering enabled (strength=50) vs disabled (default),
//! verifying that:
...
fn dithering_modifies_chroma_at_high_quality() {
    ...
    // Decode with dithering enabled (strength=50)
    let config_dithered = DecodeConfig::default().with_dithering_strength(50);
    assert_eq!(config_dithered.dithering_strength, 50);
    let (pixels_dithered, w, h) = DecodeRequest::new(&config_dithered, &webp_data)
        .decode_rgba()
        .expect("decode failed");

    // Decode with default (dithering off)
    let config_none = DecodeConfig::default();
