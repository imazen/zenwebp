//! VP8 constant tables for the coefficient transcoder.
//!
//! Values copied verbatim from zenwebp `src/common/types.rs` (RFC 6386
//! / libwebp). `pub(crate)` there → `pub` here; identical data.
#![allow(dead_code)]

pub type Prob = u8;
pub const NUM_DCT_TOKENS: usize = 12;
pub type TokenProbTables = [[[[Prob; NUM_DCT_TOKENS - 1]; 3]; 8]; 4];

// Intra prediction mode indices.
...
pub const KEYFRAME_YMODE_PROBS: [Prob; 4] = [145, 156, 163, 128];
...
pub const KEYFRAME_BPRED_MODE_PROBS: [[[Prob; 9]; 10]; 10] = [
    [
...
pub const KEYFRAME_UV_MODE_PROBS: [Prob; 3] = [142, 114, 183];

// Coeff update + default probs, token tree, cat probs, quant, zigzag,
// DCT token indices.
...
pub const COEFF_PROBS: TokenProbTables = [
    [
        [
...
pub const PROB_DCT_CAT: [[Prob; 12]; 6] = [
